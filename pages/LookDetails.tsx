  ...
  return (
    <div className="pb-12">
      <div className="relative aspect-[3/4] bg-zinc-100">
         <img 
           src={showResult ? `https://picsum.photos/seed/tryon-${id}/800/1200` : look.resultImageUrl} 
           alt="" 
           className={`w-full h-full object-cover transition-all duration-1000 ${isTryingOn ? 'blur-xl' : 'blur-0'}`} 
         />
         
         {isTryingOn && (
           <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/20 backdrop-blur-sm">
              <div className="w-16 h-16 border-4 border-white/30 border-t-white rounded-full animate-spin mb-4"></div>
              <p className="text-white text-xs font-bold uppercase tracking-widest drop-shadow-md">AI примерка...</p>
           </div>
         )}
      ...
      <div className="p-6 space-y-8">
        <div className="flex items-center justify-between">
           <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                 <img src={look.authorAvatar} alt="" className="w-5 h-5 rounded-full" />
                 <span className="text-xs text-zinc-400 font-bold uppercase tracking-wider">{look.authorName}</span>
              </div>
           </div>
        ...
        {(look.userDescription || look.aiDescription) && (
          <section className="space-y-2">
            {look.userDescription && (
              <p className="text-sm"><span className="font-bold">Автор:</span> {look.userDescription}</p>
            )}
            {look.aiDescription && (
              <p className="text-sm text-zinc-600"><span className="font-bold">AI:</span> {look.aiDescription}</p>
            )}
          </section>
        )}
        ...
        <section className="space-y-4">
           ...
           <div className="space-y-3">
              {lookProducts.map(p => (
                <div key={p.id} className="flex items-center gap-4 bg-zinc-50 p-3 rounded-2xl border border-zinc-100">
                   ...
                   <div className="flex-1">
                      ...
                      <p className="text-sm font-bold mt-0.5">{p.price} {CURRENCY}</p>
                   </div>
                   <button className="bg-zinc-900 text-white px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest">Купить</button>
                </div>
              ))}
           </div>
        </section>
        ...
        <section className="space-y-4">
           ...
           <div className="flex gap-4 overflow-x-auto no-scrollbar">
              {localLooks.slice(0, 4).map(l => (
                <Link key={l.id} to={`/look/${l.id}`} className="flex-shrink-0 w-32 aspect-square rounded-2xl bg-zinc-100 overflow-hidden">
                   <img src={l.resultImageUrl} alt="" className="w-full h-full object-cover" />
                </Link>
              ))}
           </div>
        </section>
              ...
              {comments.map((c) => (
                <div key={c.id} className="flex gap-3">
                  <div className="w-9 h-9 rounded-full bg-zinc-100 overflow-hidden">
                    {c.authorAvatar ? <img src={c.authorAvatar} className="w-full h-full object-cover" /> : null}
                  </div>
                  <div className="flex-1">
          ...
          {user?.id && (
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`bg-zinc-900 text-white px-6 py-3 rounded-full text-xs font-bold uppercase tracking-widest ${commentBusy ? 'opacity-60 pointer-events-none' : ''}`}
              >Отправить</button>
            </div>
          )}
        </section>
