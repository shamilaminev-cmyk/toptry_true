      ...
      <div className="p-4 space-y-6">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
           <button 
             ...
             className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest border transition-all ${activeCategory === 'all' ? 'bg-zinc-900 text-white border-zinc-900 shadow-md' : 'bg-white border-zinc-200 text-zinc-400 hover:border-zinc-400'}`}
           >Все</button>
           {Object.values(Category).map(cat => (
             <button 
               key={cat}
               onClick={() => setActiveCategory(cat)}
               className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest border transition-all ${activeCategory === cat ? 'bg-zinc-900 text-white border-zinc-900 shadow-md' : 'bg-white border-zinc-200 text-zinc-400 hover:border-zinc-400'}`}
             >{cat}</button>
           ))}
        </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <input value={draftTitle} onChange={(e) => setDraftTitle(e.target.value)} className="w-full border border-zinc-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-zinc-900" />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select value={draftCategory} onChange={(e) => setDraftCategory(e.target.value as any)} className="w-full border border-zinc-200 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:border-zinc-900">
                        ...
                        <div>
                          ...
                          <select value={draftGender} onChange={(e) => setDraftGender(e.target.value as any)} className="w-full border border-zinc-200 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:border-zinc-900">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input value={draftColor} onChange={(e) => setDraftColor(e.target.value)} className="w-full border border-zinc-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-zinc-900" />
                        </div>
                        <div>
                          <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Материал</label>
                          <input value={draftMaterial} onChange={(e) => setDraftMaterial(e.target.value)} className="w-full border border-zinc-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-zinc-900" />
                        </div>
                      </div>

                      <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Теги (через запятую)</label>
                        <input value={draftTags} onChange={(e) => setDraftTags(e.target.value)} className="w-full border border-zinc-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-zinc-900" />
                      </div>

                      <div className="flex gap-2">
                        <button onClick={() => setExtracted(null)} className="flex-1 border border-zinc-200 py-4 rounded-full text-xs font-bold uppercase tracking-widest">Отмена</button>
                        <button onClick={confirmAddition} className="flex-1 bg-zinc-900 text-white py-4 rounded-full text-xs font-bold uppercase tracking-widest">Добавить</button>
                      </div>
                    </div>
                  )}
                </>
        ...
        {wardrobe.length === 0 ? (
          <div className="py-24 text-center border-2 border-dashed border-zinc-100 rounded-[48px] bg-zinc-50 px-8 space-y-8">
             ...
             <button onClick={() => navigate('/catalog')} className="bg-zinc-900 text-white py-4 px-12 rounded-full text-xs font-bold uppercase tracking-widest">В каталог</button>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-4">
             {filteredItems.map(item => (
               ...
               >
                  ...
                  <button onClick={() => actions.removeFromWardrobe(item.id)} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 p-1.5 rounded-lg text-zinc-400 hover:text-red-500 shadow-sm">
        ...
        <div className="pt-6">
          <div className="bg-zinc-900 text-white p-8 rounded-[40px] space-y-6 shadow-2xl overflow-hidden relative group">
            ...
            <div className="relative z-10 space-y-2">
              ...
              <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest leading-relaxed">Используйте Gemini 3 для виртуальной примерки ваших вещей.</p>
            </div>
            <button 
