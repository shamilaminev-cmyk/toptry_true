...
export const NEVERENDING_NOVEL_CHAPTER_PLANNER_GATEWAY_VERSION =
  "neverending-novel-chapter-planner-gateway-v1";

export const NEVERENDING_NOVEL_CHAPTER_PLANNER_SCHEMA_NAME =
...
function ensureString(value, field, maximumLength) {
  ...
  if (!normalized) {
    throw createGatewayError(
      "NEVERENDING_NOVEL_CHAPTER_PLANNER_INVALID_INPUT",
      `${field} is required`,
      400
    );
  }
...
function assertStrictSchemaObjects(node, path = "$") {
  ...
  if (
    ...
    if (
      ...
      throw createGatewayError(
        "NEVERENDING_NOVEL_CHAPTER_PLANNER_INVALID_SCHEMA",
...
export function parseNeverendingNovelChapterPlannerInput(raw) {
  ...
  if (raw.role !== "chapter_planner") {
    throw createGatewayError(
      "NEVERENDING_NOVEL_CHAPTER_PLANNER_INVALID_INPUT",
      "role must be chapter_planner",
      400
    );
  }
  ...
  if (
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_CHAPTER_PLANNER_INVALID_INPUT",
      `schemaName must be ${NEVERENDING_NOVEL_CHAPTER_PLANNER_SCHEMA_NAME}`,
      400
    );
  }
...
export function describeNeverendingNovelChapterPlanner() {
  return {
    gatewayVersion:
      NEVERENDING_NOVEL_CHAPTER_PLANNER_GATEWAY_VERSION,
    provider: "openai",
    model: normalizeModel(
      process.env.NEVERENDING_NOVEL_CHAPTER_PLANNER_MODEL
    ),
    reasoningEffort: normalizeReasoningEffort(
      process.env.NEVERENDING_NOVEL_CHAPTER_PLANNER_REASONING_EFFORT
    ),
    maxOutputTokens: normalizeMaxOutputTokens(
      process.env.NEVERENDING_NOVEL_CHAPTER_PLANNER_MAX_OUTPUT_TOKENS
    ),
...
export function buildNeverendingNovelChapterPlannerRequest(
  input,
  overrides = {}
) {
  const parsed =
    parseNeverendingNovelChapterPlannerInput(input);
...
function parseCompletedChapterPlannerResponse(
  response,
  requestMetadata
) {
  ...
  if (!outputText) {
    throw createGatewayError(
      ...
      "OpenAI did not return Chapter Planner output",
      502,
      {
        providerRequestId:
          response?._request_id ?? null
  ...
  try {
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_OPENAI_INVALID_RESPONSE",
      "OpenAI Chapter Planner output is not valid JSON",
      502,
      {
        providerRequestId:
          response?._request_id ?? null
...
function validateResponseId(value) {
  ...
  if (
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_CHAPTER_PLANNER_INVALID_RESPONSE_ID",
...
function logOpenAiProviderError(
  ...
  console.error(
    `[toptry] Neverending Novel Chapter Planner OpenAI ${operation} provider error`,
    {
...
export async function startNeverendingNovelChapterPlanner(
  rawInput
) {
  const input =
    parseNeverendingNovelChapterPlannerInput(rawInput);

  const request =
    buildNeverendingNovelChapterPlannerRequest(input);
  ...
  try {
    ...
    throw createGatewayError(
      ...
      "OpenAI Chapter Planner request failed",
      502,
      {
        providerStatus:
  ...
  if (response.status === "completed") {
    return parseCompletedChapterPlannerResponse(
      response,
      request
    );
  }
...
export async function retrieveNeverendingNovelChapterPlanner(
  rawResponseId
) {
  ...
  try {
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_OPENAI_UPSTREAM_FAILED",
      "OpenAI Chapter Planner status request failed",
      502,
      {
        providerStatus:
          providerError?.status ?? null,
  ...
  throw createGatewayError(
    ...
    `Chapter Planner response reached terminal status: ${response.status}`,
    502,
    {
      providerRequestId:
        response?._request_id ?? null,
