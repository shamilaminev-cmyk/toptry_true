...
export function parsePrStudioImageReviewInput(value) {
  ...
  const normalizedCandidates = candidates.map((candidate, index) => {
    ...
    return {
      ...
      mimeType,
      data,
      origin: cleanNullableString(candidate.origin, 40),
...
export function buildPrStudioImageReviewRequest(parsed) {
  ...
  const content = [{
    ...
    text: JSON.stringify({
      ...
      candidates: parsed.candidates.map(({ id, origin, title, domain, sourceReason, sourceContext }) => ({
        id,
        origin,
        title,
        domain,
  ...
  return {
    ...
    instructions: [
      ...
      "When no themes are supplied, score topicScore directly against mainIdea and mustShow; leave bestThemeId empty rather than inventing a theme.",
      ...
      "When candidate.origin is 'generated', treat the image as synthetic editorial artwork, never as documentary evidence of a real photographed event.",
      "For a generated candidate, captionSuggestion must explicitly disclose its synthetic nature in natural editorial language, for example with the local-language equivalent of 'Illustration:' or 'Reconstruction:'. Never describe a generated scene as an archival photograph, historical photograph, documentary photograph or direct visual evidence of a real event, person, place or date.",
      "Article context may explain what a generated image is intended to represent, but it does not turn invented visual details into historical facts.",
      ...
      "When sourceContext explicitly and unambiguously confirms a depicted identity, organization, place, date or event, and that fact materially improves the editorial caption, include it directly and concisely in captionSuggestion.",
      "Prefer the most informative caption supported by both the visible pixels and candidate-specific sourceContext. Do not make an adequately sourced caption artificially generic.",
      ...
      "captionSuggestion must read as final editorial copy, not as an explanation of provenance or model reasoning. Never append phrases such as 'according to the source', 'from source attribution', 'according to metadata', 'из атрибуции источника', 'по данным источника', 'согласно метаданным' or similar evidentiary commentary. Provenance belongs in the separate source/credit fields.",
