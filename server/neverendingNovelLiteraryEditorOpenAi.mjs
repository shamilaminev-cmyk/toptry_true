...
export const NEVERENDING_NOVEL_LITERARY_EDITOR_GATEWAY_VERSION =
  "neverending-novel-literary-editor-gateway-v1";

export const NEVERENDING_NOVEL_LITERARY_EDITOR_SCHEMA_NAME =
...
function ensureString(value, field, maximumLength) {
  ...
  if (!normalized) {
    throw createGatewayError(
      "NEVERENDING_NOVEL_LITERARY_EDITOR_INVALID_INPUT",
      `${field} is required`,
      400
    );
  }
...
function assertStrictSchemaObjects(node, path = "$") {
  ...
  if (
    ...
    if (
      ...
      throw createGatewayError(
        "NEVERENDING_NOVEL_LITERARY_EDITOR_INVALID_SCHEMA",
...
export function parseNeverendingNovelLiteraryEditorInput(raw) {
  ...
  if (raw.role !== "literary_editor") {
    throw createGatewayError(
      "NEVERENDING_NOVEL_LITERARY_EDITOR_INVALID_INPUT",
      "role must be literary_editor",
      400
    );
  }
  ...
  if (
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_LITERARY_EDITOR_INVALID_INPUT",
      `schemaName must be ${NEVERENDING_NOVEL_LITERARY_EDITOR_SCHEMA_NAME}`,
      400
    );
  }
...
export function describeNeverendingNovelLiteraryEditor() {
  return {
    gatewayVersion:
      NEVERENDING_NOVEL_LITERARY_EDITOR_GATEWAY_VERSION,
    provider: "openai",
    model: normalizeModel(
      process.env.NEVERENDING_NOVEL_LITERARY_EDITOR_MODEL
    ),
    reasoningEffort: normalizeReasoningEffort(
      process.env.NEVERENDING_NOVEL_LITERARY_EDITOR_REASONING_EFFORT
    ),
    maxOutputTokens: normalizeMaxOutputTokens(
      process.env.NEVERENDING_NOVEL_LITERARY_EDITOR_MAX_OUTPUT_TOKENS
    ),
...
export function buildNeverendingNovelLiteraryEditorRequest(
  input,
  overrides = {}
) {
  const parsed =
    parseNeverendingNovelLiteraryEditorInput(input);
...
function parseCompletedLiteraryEditorResponse(
  response,
  requestMetadata
) {
  ...
  if (!outputText) {
    throw createGatewayError(
      ...
      "OpenAI did not return Literary Editor output",
      502,
      {
        providerRequestId:
          response?._request_id ?? null
  ...
  try {
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_OPENAI_INVALID_RESPONSE",
      "OpenAI Literary Editor output is not valid JSON",
      502,
      {
        providerRequestId:
          response?._request_id ?? null
...
function validateResponseId(value) {
  ...
  if (
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_LITERARY_EDITOR_INVALID_RESPONSE_ID",
...
function logOpenAiProviderError(
  ...
  console.error(
    `[toptry] Neverending Novel Literary Editor OpenAI ${operation} provider error`,
    {
...
export async function startNeverendingNovelLiteraryEditor(
  rawInput
) {
  const input =
    parseNeverendingNovelLiteraryEditorInput(rawInput);

  const request =
    buildNeverendingNovelLiteraryEditorRequest(input);
  ...
  try {
    ...
    throw createGatewayError(
      ...
      "OpenAI Literary Editor request failed",
      502,
      {
        providerStatus:
  ...
  if (response.status === "completed") {
    return parseCompletedLiteraryEditorResponse(
      response,
      request
    );
  }
...
export async function retrieveNeverendingNovelLiteraryEditor(
  rawResponseId
) {
  ...
  try {
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_OPENAI_UPSTREAM_FAILED",
      "OpenAI Literary Editor status request failed",
      502,
      {
        providerStatus:
          providerError?.status ?? null,
  ...
  throw createGatewayError(
    ...
    `Literary Editor response reached terminal status: ${response.status}`,
    502,
    {
      providerRequestId:
        response?._request_id ?? null,
