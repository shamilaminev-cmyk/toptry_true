...
export const NEVERENDING_NOVEL_ARCHITECT_GATEWAY_VERSION =
  "neverending-novel-story-architect-gateway-v1";

export const NEVERENDING_NOVEL_ARCHITECT_SCHEMA_NAME =
...
function ensureString(value, field, maximumLength) {
  ...
  if (!normalized) {
    throw createGatewayError(
      "NEVERENDING_NOVEL_ARCHITECT_INVALID_INPUT",
      `${field} is required`,
      400
    );
  }
...
function assertStrictSchemaObjects(node, path = "$") {
  ...
  if (
    ...
    if (
      ...
      throw createGatewayError(
        "NEVERENDING_NOVEL_ARCHITECT_INVALID_SCHEMA",
...
export function parseNeverendingNovelStoryArchitectInput(raw) {
  ...
  if (raw.role !== "story_architect") {
    throw createGatewayError(
      "NEVERENDING_NOVEL_ARCHITECT_INVALID_INPUT",
      "role must be story_architect",
      400
    );
  }
  ...
  if (
    ...
    throw createGatewayError(
      "NEVERENDING_NOVEL_ARCHITECT_INVALID_INPUT",
      `schemaName must be ${NEVERENDING_NOVEL_ARCHITECT_SCHEMA_NAME}`,
      400
    );
  }
...
export function describeNeverendingNovelStoryArchitect() {
  return {
    gatewayVersion:
      NEVERENDING_NOVEL_ARCHITECT_GATEWAY_VERSION,
    provider: "openai",
    model: normalizeModel(
      process.env.NEVERENDING_NOVEL_ARCHITECT_MODEL
    ),
    reasoningEffort: normalizeReasoningEffort(
      process.env.NEVERENDING_NOVEL_ARCHITECT_REASONING_EFFORT
    ),
    maxOutputTokens: normalizeMaxOutputTokens(
      process.env.NEVERENDING_NOVEL_ARCHITECT_MAX_OUTPUT_TOKENS
    ),
...
export function buildNeverendingNovelStoryArchitectRequest(
  input,
  overrides = {}
) {
  const parsed =
    parseNeverendingNovelStoryArchitectInput(input);
